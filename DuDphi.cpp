...
int main (int argc, char *argv[]) {
	...
	printf("[[%.10f,%.10f,0.0],",cos(t)/y[0],sin(t)/y[0]);
	...
	int i=0;
	double twoLoops = 2.0*2.0*M_PI;

	while (y[0] > 0.001 && y[0] < 0.5 && t< twoLoops && i<100000) {
	...
	// fclose(file);
	if (err==-1) printf("[%d,%d]]",(int)predict,err);
	else printf("[%d,%d]]",(int)predict,(int)final);
